...
def materials_composition_axes(value: object, major_fraction_threshold: float) -> list[str]:
    from pymatgen.core import Composition

    composition = Composition(str(value)).fractional_composition
    axes: list[str] = []
    for element, amount in composition.items():
        symbol = element.symbol
        fraction = float(amount)
        axes.append(f"element={symbol}")
        if fraction >= major_fraction_threshold:
            axes.append(f"major_element={symbol}")
        ...
        rows.append(
            {
                ...
                "axis": axis,
                "is_target_axis_alias": bool(axis in alias_set),
...
def summarize_group(
    ...
    n_seeds = len(seeds)
    mean_rank = float(sum(seed_best_ranks) / n_seeds) if n_seeds else float("nan")
    mean_score = float(sum(seed_target_scores) / n_seeds) if n_seeds else float("nan")
    return {
        "dataset": dataset_name,
        "model": model,
        "mode": mode,
        ...
        "seed_target_best_rank_mean": mean_rank,
        "seed_target_conflict_score_mean": mean_score,
...
def main() -> int:
    config_path = parse_config_arg("Analyze B35 hypothesis-axis recovery from conflict traces.")
    cfg = load_json_config(config_path)
    require_keys(cfg, REQUIRED_CONFIG_KEYS, "b35_hypothesis_axis_recovery")
