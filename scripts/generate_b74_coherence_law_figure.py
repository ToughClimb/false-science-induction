...
def plot_coherence_to_budget(axis: plt.Axes, materials: pd.DataFrame, gfp: pd.DataFrame) -> None:
    ...
    axis.fill_between(x_material, 0, y_material, color=OKABE_ITO["vermillion"], alpha=0.10)
    axis.fill_between(x_gfp, 0, y_gfp, color=OKABE_ITO["blue"], alpha=0.08)
    ...
    axis.legend(loc="upper left", fontsize=6.7, handlelength=1.4)
    style_axis(axis)
    panel_label(axis, "a")
...
def plot_risk_law(axis: plt.Axes, frame: pd.DataFrame) -> None:
    ...
    axis.legend(loc="lower right", fontsize=6.5, handlelength=1.2)
    style_axis(axis)
    panel_label(axis, "c")
