...
def write_tex(path: Path, screen_summary: list[dict[str, object]], bear_summary: list[dict[str, object]]) -> None:
    ...
    lines = [
        ...
        "\\centering",
        "\\small",
        "\\caption{Reviewer-gap closure checks: standard screens and BEAR feedback triage.}",
        "\\label{tab:b73-reviewer-gap-closure}",
        "\\resizebox{\\linewidth}{!}{%",
...
def main() -> int:
    config_path = parse_config_arg("Run B73 reviewer-gap closure analyses.")
    cfg = load_json_config(config_path)
    require_keys(cfg, REQUIRED_CONFIG_KEYS, "b73_reviewer_gap_closure")
