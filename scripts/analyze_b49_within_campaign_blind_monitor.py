...
def empirical_axis_threshold(
    ...
    if not early_axis_rows.empty:
        values.extend(early_axis_rows["axis_selected_fraction"].astype(float).tolist())
    if not peer_axis_rows.empty:
        values.extend(peer_axis_rows["axis_selected_fraction"].astype(float).tolist())
    if not values:
        return float("inf")
...
def annotate_flags(
    ...
    for (_, _, _, _), group in grouped:
        ...
        for row in evaluation.to_dict("records"):
            ...
            annotated = dict(row)
            annotated["calibration"] = "within_campaign"
            annotated["threshold_source"] = "early_same_axis_plus_same_round_peer_axes"
            annotated["axis_threshold"] = threshold
            annotated["flagged"] = bool(float(row["axis_selected_fraction"]) > threshold)
            rows.append(annotated)
    if not rows:
        return pd.DataFrame()
    flagged = pd.DataFrame(rows)
    flagged = flagged.sort_values(
        ["dataset", "model", "mode", "seed", "round", "flagged", "axis_selected_fraction", "axis"],
...
def rows_for_dataset(
    ...
    grouped = selected.groupby(["mode", "seed", "round"], sort=True)
    for (mode, seed, round_idx), group in grouped:
        axis_rows.extend(
            trace_axis_rows(
...
def main() -> int:
    config_path = parse_config_arg("Analyze B49 within-campaign blind monitor.")
    cfg = load_json_config(config_path)
    require_keys(cfg, REQUIRED_CONFIG_KEYS, "b49_within_campaign_blind_monitor")
    baseline_rounds = require_int_list(cfg, "baseline_rounds", "b49_within_campaign_blind_monitor")
    evaluation_rounds = require_int_list(
        cfg,
        "evaluation_rounds",
        "b49_within_campaign_blind_monitor",
    )
