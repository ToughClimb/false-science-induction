...
def main() -> int:
    config_path = parse_config_arg("Aggregate materials dose-response summaries.")
    cfg = load_json_config(config_path)
    require_keys(cfg, REQUIRED_CONFIG_KEYS, "aggregate_materials_dose_response")
    run_dirs = require_string_list(cfg, "run_dirs", "aggregate_materials_dose_response")
    required_metadata_keys = require_string_list(
        cfg,
        "required_metadata_keys",
        "aggregate_materials_dose_response",
    )
    ...
    for run_dir_text in run_dirs:
        ...
        metadata = read_metadata(run_dir / metadata_file, required_metadata_keys)
        summary = read_summary(run_dir / summary_file, required_summary_columns)
        summary.insert(0, "run_dir", str(run_dir))
        for index, key in enumerate(reversed(required_metadata_keys), start=1):
            summary.insert(index, key, metadata[key])
